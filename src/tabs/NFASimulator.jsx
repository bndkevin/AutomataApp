...
import { Play, Pause, SkipForward, SkipBack, RefreshCw, CheckCircle, XCircle, HelpCircle } from 'lucide-react';
...
export default function NFASimulator() {
  const [nfa, setNfa] = useState(EXAMPLE_NFA); // Load example by default so user doesn't see empty
  const [testString, setTestString] = useState("abaa");
  ...
  // Load standard pre-fill NFA example
  const handleLoadExample = () => {
    setNfa(JSON.parse(JSON.stringify(EXAMPLE_NFA)));
    setTestString("abaa");
  ...
  // Run automatically when NFA inputs change if we already simulated once
  useEffect(() => {
    if (simulationResult) {
      const res = runNFA(nfa, testString);
      setSimulationResult(res);
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 max-w-7xl mx-auto w-full p-4 flex-1">
      {/* LEFT COLUMN: Configuration (NFA Input Form) */}
      <div className="xl:col-span-5 flex flex-col gap-6">
        <NFAInputForm
          value={nfa}
          onChange={setNfa}
          onLoadExample={handleLoadExample}
          exampleLabel="Mengandung 'aa'"
          title="Desain Nondeterministic Finite Automata (NFA)"
        />
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 text-xs text-slate-400 leading-relaxed flex items-start gap-3">
          ...
          <div>
            <h4 className="font-bold text-slate-200 mb-1">Panduan Konfigurasi NFA:</h4>
            <p>Masukkan state yang unik (misal: q0, q1, q2). Tentukan alfabet simbol input (misal: a, b). Pilih State Awal dan State Penerima (Accept). Isi tabel transisi dengan memisahkan target dengan koma (misal: q0, q1) atau biarkan kosong/Ø jika tidak ada transisi. Kolom ε mewakili transisi epsilon.</p>
          </div>
        </div>
      </div>

      {/* RIGHT COLUMN: Graph Visualizer + NFA Simulation Results */}
